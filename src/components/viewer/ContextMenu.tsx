...
export function ContextMenu({ menu, onClose, onHide, onIsolate, onShowAll, onFitSelected }: ContextMenuProps) {
  ...
  return (
    ...
    >
      {menu.expressId !== null && (
        <>
          <button type="button" onClick={() => { onHide(menu.expressId!); onClose(); }}>
            <EyeOff size={14} />
            <span>Hide</span>
          </button>
          <button type="button" onClick={() => { onIsolate(menu.expressId!); onClose(); }}>
            <Layers size={14} />
            <span>Isolate</span>
          </button>
          <button type="button" onClick={() => { onFitSelected(); onClose(); }}>
            <Focus size={14} />
