...
function flattenResidentMeshes(
	chunksById: Record<number, RenderChunkPayload>,
	residentChunkIds: number[],
) {
...
class ViewportGeometryStore {
	...
	private _cachedMeshes: TransferableMeshData[] = [];
	...
	getCachedMeshes = () => this._cachedMeshes;
	...
	upsertChunks = (chunks: RenderChunkPayload[]) => {
		...
		this._cachedMeshes = flattenResidentMeshes(
			nextChunksById,
			residentChunkIds,
		);
		this.emit();
	...
	releaseChunks = (chunkIds: number[]) => {
		...
		this.snapshot = {
			...
			chunksById: nextChunksById,
			residentChunkIds,
			visibleChunkIds: this.snapshot.visibleChunkIds.filter(
