...
| 현재 단계 | Production | Phase 5.5 Stabilization |
...
| 2.15 | Model header UI | ❌ | 멀티모델 대응 시 필요 |
...
| 2.19 | Row height/font density 미세 조정 | ❌ | UX polish |
...
| 3.8 | Property cache | 🟡 | 동일 엔티티 재선택 시 미캐시 |
...
| 16.2 | ViewCube (3D 네비게이션 큐브) | ❌ | ViewCube |
| 16.3 | Axis Helper | ❌ | AxisHelper |
| 16.4 | Entity Context Menu (우클릭) | ❌ | EntityContextMenu |
| 16.5 | Hover Tooltip | ❌ | HoverTooltip |
| 16.6 | Keyboard Shortcuts Dialog | ❌ | KeyboardShortcutsDialog |
...
| 16.11 | Progress indicator (file load) | 🟡 | 기본 로딩 상태만 |
...
기존 `implementation-roadmap.md`와 일치하는 우선 작업. 현재 Phase 5.5에서 자연스럽게 이어지는 항목.
